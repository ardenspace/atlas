...
```bash
cd web && bun run dev   # 5173, /api → 8787 프록시
```

React+Vite+TS UI. `web/dist`가 없으면(빌드 전) 비-API 경로는 500이 난다 — `/api/*`는 그대로 동작하고, `bun run build` 후 정상 서빙된다.
...
```bash
uv run pytest                              # 백엔드
cd web && bun run typecheck && bun run test    # 프론트 (vitest+RTL+MSW; bun test 아님)
```
