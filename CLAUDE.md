...
- `web/` — React+Vite+TS(strict) UI. 3판 워크스페이스(사이드바/챗/문서 패널) +
  문서 편집·정착 오버레이. 패키지 매니저는 bun (`bun install`, `bun.lock`).
  dev는 5173(`cd web && bun run dev`, `/api`→8787 프록시),
  프로덕션은 `bun run build` 후 FastAPI가 `web/dist`를 서빙 (빌드 전엔 dist가 없어
  비-API 경로 500 — 서버·`/api/*`는 정상. dist가 있고 경로만 없으면 404).
  테스트: `cd web && bun run test` — vitest+RTL+MSW, 모킹 누락 시 즉시 실패(onUnhandledRequest:
  'error')라 8787/8080을 절대 치지 않는다. (`bun test`가 아니라 `bun run test` — 전자는 bun 내장 러너를 불러 vitest 설정을 안 탄다.)
- 테스트: `uv run pytest` + `cd web && bun run typecheck && bun run test` — llama-server를 절대 직접 치지 않는다 (모킹/MockTransport).
